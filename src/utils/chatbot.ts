...
const findFacultyInQuery = (query: string): Faculty | undefined => {
  ...
  const sortedFaculties = [...LASU_KNOWLEDGE_BASE].sort(
    (a, b) => b.faculty.length - a.faculty.length,
  );
  ...
  for (const f of sortedFaculties) {
    ...
    if (
      norm.includes(name) ||
      (shortName.length > 3 && norm.includes(shortName))
    ) {
      return f;
    }
  }
...
const findDepartmentInQuery = (
  query: string,
): { dept: Department; faculty: Faculty } | undefined => {
  const norm = query.toLowerCase();
...
const findPoiInQuery = (query: string, pois: POI[]): POI | undefined => {
  ...
  if (norm.includes("law library") || norm.includes("elias")) {
    const lawLib = pois.find(
      (p) =>
        p.name.toLowerCase().includes("elias") ||
        p.name.toLowerCase().includes("law library"),
    );
    if (lawLib) return lawLib;
  }
  if (
    norm.includes("sanwo-olu") ||
    norm.includes("sanwo olu") ||
    norm.includes("sanwoolu") ||
    norm.includes("library complex") ||
    norm.includes("new library") ||
    norm.includes("central library") ||
    norm.includes("reading room") ||
    norm.includes("study area")
  ) {
    const sanwoOluLib = pois.find(
      (p) =>
        p.name.toLowerCase().includes("sanwo-olu") ||
        p.name.toLowerCase().includes("sanwo olu") ||
        p.id === "26",
    );
  ...
  if (norm.includes("library")) {
    const mainLibrary = pois.find(
      (p) => p.name.toLowerCase().includes("akesode") || p.id === "4",
    );
...
const formatPoiLocationResponse = (
  poi: POI,
  pois: POI[],
): { text: string; poi?: POI } => {
  let nearbyText = "";
...
const formatFacultyResponse = (
  faculty: Faculty,
  pois: POI[],
  poi?: POI,
  includeProgrammes = false,
): { text: string; poi?: POI } => {
  let nearbyText = "";
  if (poi) {
    const landmarks =
...
const formatFacultyDepartmentsResponse = (
  faculty: Faculty,
  poi?: POI,
): { text: string; poi?: POI } => {
  const deptsList = faculty.departments.map((d) => `• ${d.name}`).join("\n");
...
const formatFacultyProgrammesResponse = (
  faculty: Faculty,
  poi?: POI,
): { text: string; poi?: POI } => {
...
const formatFacultyLocationResponse = (
  faculty: Faculty,
  poi?: POI,
  pois: POI[] = [],
): { text: string; poi?: POI } => {
...
export const processQuery = (
  ...
  if (normalized.includes("dean")) {
    ...
    normalized.includes("how to get to") ||
  ...
  if (intent === "dean") {
    const targetFaculty =
      facultyMatch || (deptMatch ? deptMatch.faculty : undefined);
    if (targetFaculty) {
      return {
        text: `The Dean of the ${targetFaculty.faculty} is ${targetFaculty.dean}.`,
      };
    }
    return {
      text: "I could not find the dean's information for the requested faculty.",
    };
  }
  ...
  if (intent === "departments") {
    ...
    return {
      text: "I could not find the departments for the requested faculty.",
    };
  }
  ...
  if (intent === "programmes") {
    ...
    return {
      text: "I could not find the programmes offered by the requested faculty.",
    };
  }
  ...
  if (poiMatch) {
    const matchedFaculty = LASU_KNOWLEDGE_BASE.find(
      (f) =>
        f.faculty.toLowerCase().includes(poiMatch.name.toLowerCase()) ||
        poiMatch.name.toLowerCase().includes(f.faculty.toLowerCase()) ||
        poiMatch.searchAliases?.some(
          (a) => a.toLowerCase() === f.abbreviation.toLowerCase(),
        ),
    );
    if (matchedFaculty) {
      if (intent === "location") {
  ...
  for (const f of LASU_KNOWLEDGE_BASE) {
    const fuzzyDept = f.departments.find(
      (d) =>
  ...
  const fuzzyFaculty = LASU_KNOWLEDGE_BASE.find(
    (f) =>
      isFuzzyMatch(normalized, f.faculty) ||
      isFuzzyMatch(normalized, f.abbreviation) ||
      isFuzzyMatch(
        normalized,
        f.faculty.replace(/faculty of|school of/g, "").trim(),
      ),
  );
  if (fuzzyFaculty) {
