...
export const POIInfo: React.FC<POIInfoProps> = ({
  ...
  const images =
    poi.imageUrls && poi.imageUrls.length > 0
      ? poi.imageUrls
      : poi.imageUrl
        ? [poi.imageUrl]
        : [];
      ...
      >
        ...
        >
          <div>
            ...
            <h2 className="text-xl font-black text-zinc-950 leading-tight tracking-tight">
              {poi.name}
            </h2>
          </div>
        ...
        {poi.description && (
          <motion.p
            ...
            className="text-zinc-900 text-xs mb-4 leading-relaxed font-semibold"
          >
            {poi.description}
          </motion.p>
        )}
        ...
        {poi.nearbyLandmarks && poi.nearbyLandmarks.length > 0 && (
          <motion.div
            ...
            className="mb-4 bg-zinc-50 border border-zinc-200 rounded-2xl p-4 space-y-2"
          >
            ...
            <div className="flex flex-wrap gap-1.5">
              {poi.nearbyLandmarks.map((landmark, idx) => (
                <span
                  key={idx}
                  className="bg-white border border-zinc-200 text-zinc-755 text-[10px] font-extrabold px-2.5 py-1 rounded-xl shadow-sm leading-none"
                >
                  {landmark}
                </span>
              ))}
        ...
        {departments && departments.length > 0 && (
          <motion.div
            ...
            className="mb-5 bg-zinc-50 border border-zinc-200 rounded-2xl p-4 space-y-3"
          >
            <div className="flex items-center gap-2 border-b border-zinc-200/30 pb-2">
              <svg
                ...
                stroke="currentColor"
                strokeWidth="2.5"
                className="w-4 h-4 text-lasu-primary"
              >
