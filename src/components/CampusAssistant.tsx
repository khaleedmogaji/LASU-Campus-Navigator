        ...
        <button
          ...
          className={cn(
            ...
            isOpen
              ...
              : "bg-lasu-accent text-white shadow-md hover:scale-105",
          )}
        ...
        {isOpen && (
          <motion.div
            ...
            className={cn(
              ...
              "bg-white shadow-2xl ring-1 ring-black/5 rounded-3xl h-[480px] flex flex-col overflow-hidden",
            )}
          >
            {/* Header */}
            <div className="p-4 flex items-center justify-between bg-gradient-to-r from-lasu-accent/10 to-transparent border-b border-zinc-100 shrink-0">
              ...
              <button
                ...
                className="p-1.5 hover:bg-zinc-200/70 rounded-full text-zinc-500 hover:text-zinc-700 transition-colors cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 flex flex-col custom-scrollbar">
              {messages.map((msg) => (
                ...
                >
                  <div
                    className={cn(
                      "px-3.5 py-2.5 rounded-2xl font-semibold whitespace-pre-line leading-relaxed",
                      msg.sender === "user"
                        ? "bg-lasu-accent text-white rounded-tr-sm"
                        : "bg-zinc-50 text-[rgb(49,30,2)] rounded-tl-sm",
                    )}
                  >
                    {msg.text}
                  </div>
                  ...
                  {msg.poi && (
                    <button
                      ...
                      className={cn(
                        ...
                        "flex items-center gap-1.5 font-black uppercase text-[10px] hover:scale-102 transition-all duration-200 active:scale-95 cursor-pointer",
                      )}
                    >
              ...
              {isTyping && (
                <div className="flex flex-col gap-1 text-xs self-start items-start">
                  <div className="px-3.5 py-2.5 rounded-2xl bg-zinc-50 rounded-tl-sm flex items-center gap-1">
                    <span
            ...
            <div className="px-3 py-2 bg-zinc-50 border-t border-zinc-100 flex gap-2 overflow-x-auto scrollbar-none shrink-0 max-w-full">
              {SUGGESTED_CHIPS.map((chip, idx) => (
                <button
                  key={idx}
                  type="button"
                  onClick={() => handleChipClick(chip)}
                  className="px-3 py-1.5 bg-white border border-transparent hover:border-lasu-accent/20 rounded-full text-[10px] font-bold text-zinc-600 hover:text-lasu-accent whitespace-nowrap active:scale-95 transition-all shadow-sm shrink-0 cursor-pointer"
                >
                  {chip}
                </button>
              ))}
            ...
            <form
              onSubmit={handleSend}
              className="p-3 border-t border-zinc-100 bg-zinc-50 flex gap-2 shrink-0"
            >
              <input
                type="text"
                value={inputValue}
                ...
                className="flex-1 bg-white border border-zinc-200 rounded-xl px-3 py-2.5 text-xs font-semibold text-zinc-700 placeholder-zinc-400 focus:outline-none focus:border-lasu-accent focus:ring-2 focus:ring-lasu-accent/10"
              />
              <button
                type="submit"
                ...
                className={cn(
                  ...
                  inputValue.trim()
                    ? "bg-lasu-accent hover:opacity-90 border-lasu-accent text-white shadow-md active:scale-95"
